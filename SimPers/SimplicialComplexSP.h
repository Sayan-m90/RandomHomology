...
public:
	...
	void remove_simplex_from_both_complex_and_ufdForest(SimplicialTreeNode_ptr sigma);
	...
	void SnapshotHomologicalFeatures(vector<unordered_set<int> > &hom_info);
	...
	void CheckPersistence(vector<unordered_set<int> > &homo_info);
	//
...
private:
	...
	void delete_from_circular_list(SimplicialTreeNode_ptr &sigma, const int simplex_dim) {
		unordered_map<int, SimplicialTreeNode_ptr> &head = (*labels_dict_in_each_dim[simplex_dim]);
...
SimplicialTreeNode_ptr SimplicialTree<T>::insert_into_simplicial_tree(std::vector<int> &simplex_vertices) {
	...
	if (pIter->children.find(simplex_vertices.back()) == pIter->children.end()) {
		...
		SimplicialTreeNode_ptr simplex(boost::make_shared<SimplicialTreeNode>(simplex_vertices.back(), -1));
...
SimplicialTreeNode_ptr SimplicialTree<T>::ElementaryInsersion(std::vector<int> &simplex_vertices) {
	...
	if (!simplex) {
		...
		if (simplex_dim == 0) {
			...
			annotations[0]->create_cocycle(root, ufd); // connect tree_root_node with annotation
				//   
			//check_status();
		}
		else {  
			...
			for (int i = 1; i < boundaries.size(); ++i){
                ListNodePtr p = find_annotation(boundaries[i]);
				dead_bit = annotations[simplex_dim - 1]->sum_two_annotation_with_changed_dst(sum, p);
			} 
			//
			if (dead_bit == -1) {
				...
				//		create a new cocyle
				TreeRootNodePtr root = ufd.MakeSet(simplex); // connect simplex with tree_node
				annotations[simplex_dim]->create_cocycle(root, ufd, annotations[simplex_dim]->empty()); // connect tree_root_node with annotation
				//
			}
			else {
...
template<typename T>
void SimplicialTree<T>::remove_simplex_from_both_complex_and_ufdForest(SimplicialTreeNode_ptr sigma){
	// Preq: all cofaces are deleted already
	// remove the tree node in union-find-deletion data structure
	if (!sigma->children.empty()) {
		cout << "can not be deleted as some cofaces exists" << endl;
...
void SimplicialTree<T>::PerformSimplicialCollapse(vector<pair<int, int> > &vertex_map, unordered_map<int, int> &updated_vertex_map) {
	...
	unordered_map<int, vector<int> > preimages;
	for (int i = 0; i < vertex_map.size(); ++i) {
		if (preimages.find(vertex_map[i].second) == preimages.end()) {
			preimages[vertex_map[i].second] = vector<int> (1, vertex_map[i].first);
		}
		else { 
			preimages[vertex_map[i].second].push_back(vertex_map[i].first);
		}
	} 
	for (unordered_map<int, vector<int> >::iterator mIter = preimages.begin(); mIter != preimages.end();
		++mIter) {
		...
		if (mIter->second.size() > 1) {
			...
			//} 
			for (int i = (int)mIter->second.size() - 1; i > 0; --i) {
				//check_status();
				ElementaryCollapse((mIter->second)[i], mIter->second.front());
				//cout << "after " << endl;
				//check_status();
			}
			 
		}
		updated_vertex_map[mIter->second.front()] = mIter->first; 
	}
	return;
}
template <typename T>
