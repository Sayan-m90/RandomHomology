...
public:
	/*constructor */
	// default constructor
	SimplicialTree() : dim(-1), EuclideanDataPtr(NULL), accumulativeSimplexSize(0)
	{
		...
		*/
		...
		if (getline(sstr, line)) {
			...
			while (getline(sstr, line)) {
				...
				if (simplex_count > 0 && !simplex_vertices.empty()) {
					--simplex_count;
					if ( simplex_vertices.size() == 4 && simplex_vertices[0] == 526 && simplex_vertices[1] == 749
						&& simplex_vertices[2] == 1045 && simplex_vertices[3] == 2127)
						int a = 4;
					ElementaryInsersion(simplex_vertices);
				}
				line.clear();
