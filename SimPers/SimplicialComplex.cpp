													...
													const char* new_range_complex_file_name = NULL) 
{
	...
	//ofstream myfile;
	//ofstream myfile1;
  	//myfile.open ("fromRandom.txt",ios::in);
  	//myfile1.open ("fromRandomout.txt",ios::out);
	//myfile1 << file_name_of_domain_complex<<" # "<<is_domain_complex_with_annotation<<" # "<<filtration_step<<" # "<<is_save_range_complex_with_annotation<<"\n";
  	//for (int i = 0; i < vecElemOpers.size(); ++i)
	//{		
		//myfile>>vecElemOpers[i];//endl;
	//	myfile1<<vecElemOpers[i]<<endl;
	//}
	//myfile<<"\n\n\n";
  	//myfile.close();
  	//myfile1.close();
  	//cout<<"check myfile"; getchar();
	if (filtration_step == 0)
	{
		//accumulativeSize = 0;
	...
	for (int i = 0; i < vecElemOpers.size(); ++i)
	{
		...
		else if (sOperation.compare("collapse") == 0)
		{
			...
			{
				//if(removeVertices[i] == 31)
				//	{cout<<"\nremoveVertices[i] "<<removeVertices[i]<< "preserveVertex "<<preserveVertex;	getchar();}
